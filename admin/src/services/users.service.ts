...
export const registerUser = async ({ profileName, firstName, lastName, email, password }: registerData) => {
  try {
    ...
    if (userExists) throw new Error('An account with the email address already exists.');
    ...
    const userId = await datastore.save({ namespace: 'admin', kind: 'users', entity: userDatabaseData });
